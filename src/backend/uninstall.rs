...
/// Execute a system-level pacman uninstall asynchronously
pub fn uninstall_async(
    ...
    let _ = tx.send(format!("Starting uninstallation for {}...", package.name));
    ...
    std::thread::spawn(move || {
        ...
        cmd.args(["pacman", "-Rns", "--noconfirm", &package_id]);
                ...
                for (frame_idx, line) in reader.lines().enumerate() {
                    if let Ok(l) = line {
                        if config.beta_mode {
                            ...
                            let _ = tx_out.send(format!("CLRLINE_ILOVECANDYUninstalling... [{}]", frames[frame_idx % frames.len()]));
                        } else {
                            let _ = tx_out.send(l);
                        }
                    }
        ...
        match child.wait() {
            Ok(status) => {
                if status.success() {
                    let _ = tx.send(String::from("\nUninstallation Complete! [Press Esc or Enter to return]"));
                } else {
                    let _ = tx.send(format!("\nUninstallation failed with status: {} [Press Esc to return]", status));
                }
            }
            Err(e) => {
